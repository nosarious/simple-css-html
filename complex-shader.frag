...
void main() {
    ...
    vec2 st = ( gl_FragCoord.xy / u_resolution.xy ) * 5.0 - 1.0;
    st.x *= u_resolution.x / u_resolution.y;
    
    
    float r = voronoi( st*5.0);// * 5.50;
	//float xWaves = smoothstep(0.1,1.0,sin(st.x*2.0+u_time)*2.0);
	//float yWaves = smoothstep(0.1,1.0,sin(st.x*3.0+u_time*0.250)*2.0);
	float f = fract(st.x*.50+u_time);
	float t =voronoi( st*8.0+u_time) * 1.50;
